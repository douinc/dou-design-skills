...
```python
APPENDIX = [("A1", "업무분장"), ("A2", "수행 실적"), ("A3", "사업비"), ("A4", "위험관리")]

def s_agenda():
    s = slide("발표 순서")
    agenda(s, SECTIONS, N(), note="부록 A1~A4  " + " · ".join(t for _, t in APPENDIX))

def s_apx_div():
    s = slide("부록")
    appendix_divider(s, APPENDIX)          # 제목을 바꾸려면 title="부록 · 참고자료"

def s_a1():
    s = slide("A1 업무분장")
    s.add(apx_header("A1", "업무분장", "기관별 책임 범위를 정리합니다."))
    # ... 본문은 다른 레시피와 동일 (표 위주가 많음) ...
    s.add(footer(N()))

BUILD = [s_cover, s_agenda, ..., s_closing, s_apx_div, s_a1, ...]
```
- 부록 장도 본문과 같은 규칙(넘침 없음, 아래까지 채움)을 따르되, 질의응답 때 찾아 보는 자료라 표 밀도는 본문보다 높아도 됩니다(행 8~10개까지).
- 부록 번호(A1…)와 간지 목록, 목차 note 의 번호가 서로 맞는지 마지막에 확인합니다.
