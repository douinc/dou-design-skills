...
6. **선과 글을 걷어낸다.** 화면에 그은 선·테두리마다 "이게 없으면 정말 헷갈리나"를 묻고, 안내
   문장마다 "이걸 지우면 못 하게 되나"를 묻는다. 답이 아니면 지운다 — 넘기기 전에 지우는 것이
   담당자가 빼라고 말하게 두는 것보다 낫다(DESIGN.md §1 두 원칙).

**선과 글 걷어내기 (필수).** 담당자가 가장 자주 되돌린 것이 이 둘이다 — 굳이 필요 없는 자리에
선으로 칸을 나누고, 읽지도 않을 설명을 덧붙이는 것. 지을 때는 "구분이 필요하니 선을 긋고,
헷갈릴까 봐 한 줄 더 쓴다"가 자연스러워 보여서 스스로는 잘 안 걸린다.

| 걷어낼 것 | 어떻게 판단하나 |
|---|---|
| 이미 면·모양을 가진 것 위의 선 | 입력창·카드·버튼 줄은 그 자체가 경계다. 위에 선을 더 그으면 창이 두 칸으로 나뉘어 보인다 |
| 섹션마다 그은 구분선 | 먼저 여백만으로 나눠 본다. 여백으로 갈리면 선은 없어도 된다 |
| 화면이 이미 말하는 설명 | 라벨·자리·순서가 하는 말을 문장으로 또 하지 않는다 |
| 안 읽을 안내 문장 | 지웠을 때 사용자가 못 하게 되는 일이 없으면 없어도 되는 문장이다 |
...
| ㄴ **주의** | 네 항목 **전부** 작업 순서를 적는 자리가 아니다 — 아래 `노트는 결과만 적는다` 참조 |
...
### 노트는 결과만 적는다 — 작업 순서를 나열하지 않는다 (필수)

**네 항목 전부에 걸린다.** `의도`만의 규칙이 아니다 — `동작 규칙`·`움직임`·`제약 · 예외`도
같다. 노트는 **지금 이 화면이 어떤 규칙을 갖는지**를 적는 자리고, 만드는 동안 무엇을 먼저
해보고 무엇으로 바꿨는지는 어느 항목에도 적지 않는다.

| 항목 | 이렇게 쓰지 말고 | 이렇게 |
|---|---|---|
| 동작 규칙 | 전사가 접힌 채로도 흐르던 걸 **막았다** | 전사는 펼쳐져 있을 때만 흐른다 |
| 움직임 | 크기를 화면에서만 줄였더니 부스러기가 남아서 **진짜 크기로 바꿨다** | 크기는 진짜로 바꾼다. 화면에서만 줄이면 그 부품이 자기 폭을 줄어든 값으로 알고 그만큼만 지운다 |
| 제약 · 예외 | 정책과 부딪혀서 **되돌렸다** | 접어도 저장·복사·전송 경로는 그대로다. 이 장치는 표시만 접는다 |
...
| 하단의 파형을 **걷었다** | 하단 줄에는 파형을 두지 않는다 |
| 문구에서 `전사`를 걷고 `기록 대기 중`으로 **바꿨다** | 시작 전 화면은 `기록 대기 중`이다 |
| 유리 구체를 한 번 **얹어 봤는데** 볼 것이 너무 많았다 | 스스로 형태가 도는 물체는 쓰지 않는다. 진료실에서 가장 눈에 띄는 것이 되면 이 창의 전제가 깨진다 |

### 가려내는 법 — 손으로 훑지 말고 **찾아서** 고친다 (필수)

이 규칙은 눈으로만 확인하면 반드시 샌다. 노트를 쓰는 시점은 작업이 한참 진행된 뒤라,
그때는 이 절을 다시 읽지 않고 방금 한 일이 그대로 문장이 된다. **노트를 저장한 직후
아래를 실행한다.**

```bash
grep -nE '처음에는|했다가|바꿨다|바꿨어|고쳤다|고쳤어|걷었다|없앴다|지웠다|빼|추가했다|넣었다|넣어 봤|얹어 봤|해봤더니|해보니|반영했|피드백을 받아|검토하면서|결과적으로|~로 바뀌었다' \
  <노트 파일>
```

걸린 줄은 **결과 문장으로 바꾼다.** 규칙은 하나다 — 동사를 **현재형**으로 놓고 다시 쓴다.
`걷었다` → `두지 않는다`, `바꿨다` → `~이다`, `없앴다` → `쓰지 않는다`.

**`grep` 은 노트 파일 전체에 돌린다** — 항목을 가려 돌리지 않는다. 과정 서술은 `의도`보다
`동작 규칙`·`움직임`에서 더 자주 샌다. 그 둘이 바로 방금 고친 것을 적는 자리라서다.

**한 줄도 안 걸렸다고 끝난 게 아니다.** 과거형 없이도 과정이 섞이는 형태가 둘 있다.

| 형태 | 예 | 어떻게 |
|---|---|---|
| **판 비교를 이야기로 씀** | `v1은 전사 글자를 흘렸다. v2는 파형으로 바꿨다` | 달라진 점은 남기되 **지금 판의 규칙**으로 쓴다 — `[v1과 다른 점] 녹음 중 본문의 기본은 파형이다. 전사는 켜야 보인다` |
| **원인을 나에게 둠** | `~해서 이렇게 정했다` · `~라고 판단해` | 주어를 화면·사용자로 바꾼다 — `이 창은 다른 앱 위에 떠 있어서 …` |

노트를 읽는 사람은 **지금 화면을 다시 만들 사람**이지, 우리가 어떤 순서로 일했는지 궁금한
사람이 아니다.
...
- **화면에 지난 판이 남아 있으면 노트도 판마다 따로 쓴다.** 한 노트에 여러 판의 규칙이
  섞이면 v1을 열어 놓고 v2에서 정한 규칙을 읽게 되고, 그러면 지난 판을 눌러보는 이유가
  사라진다. 새록은 `app/{제품}/screens/versions.ts` 의 `notes`.
- **각 판의 노트는 직전 판에서 달라진 점으로 시작한다.** 그게 판을 나눈 이유이자, 판을
  눌러보는 사람이 가장 먼저 찾는 것이다. 다만 **지금 판의 규칙으로** 쓴다 —
  `[v1과 다른 점] 녹음 중 본문의 기본은 파형이다. 전사는 켜야 보인다.` ✅ /
  `v1은 전사 글자를 흘렸는데 v2에서 파형으로 바꿨다` ❌. 지난 판 쪽에는 거꾸로,
  **다음 판으로 넘어가게 만든 한계**를 경계로 남긴다(`[v2로 넘어간 이유] …`).
...
4. **설계 노트에 과정 서술이 남았는지 찾는다** — ⑨ `가려내는 법`의 `grep` 을 이번에 손댄
   노트 파일 전부에 돌린다. 노트는 피드백을 반영할 때마다 자라고, 자라는 자리가 바로
   방금 한 일이 문장이 되는 자리다. 커밋 전에 한 번 더 훑지 않으면 그대로 남는다.
...
| 눈으로만 확인하고 넘김 | "과정 서술은 안 썼다"고 여기고 저장 | 이 규칙은 눈으로 보면 반드시 샌다. ⑨ `가려내는 법`의 `grep` 을 **저장 직후와 커밋 전** 두 번 돌린다 |
| 판별 노트에 판 비교를 서술 | v2 노트에 `v1은 ~였는데 v2에서 ~로 바꿨다` | 달라진 점은 남기되 지금 판의 규칙으로 쓴다 — `[v1과 다른 점] 본문의 기본은 파형이다` |
