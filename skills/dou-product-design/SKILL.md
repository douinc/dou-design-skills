...
## ②-0 그리기 전 관문 (필수 · 답이 없으면 그리지 않는다)

화면부터 그리기 시작하면 "예쁘게는 됐는데 쓸 수 없는" 결과가 나온다. 그림보다 먼저 아래
다섯 개에 답한다. 길게 쓸 필요 없다 — 각 한두 줄이면 되고, 답을 스펙 문서에 남긴다.

| 질문 | 답이 없으면 생기는 일 |
|---|---|
| **누가 · 언제 · 어떤 상황에서** 이 화면을 여나 | 진료 중 한 손인지 책상 앞 두 손인지에 따라 자리와 크기가 달라진다 |
| 이 화면에서 **성공은 무엇**인가. 몇 번 눌러야 끝나나 | 성공을 안 정하면 기능만 늘어난다 |
| 이 화면에 있을 수 있는 **상태를 다 적으면 몇 개**인가 (기본·빈·로딩·실패·권한 없음·부분 실패) | 정상만 그린 화면이 나온다 |
| **실패하면 어디로** 가나. 되돌릴 수 있나 | 막다른 길이 생긴다 |
| 이 화면이 **다루지 않는 것**은 무엇인가 | 범위가 흐려져 화면이 비대해진다 |

세 번째는 목록으로 적는다. 그 목록이 곧 ③ 와이어프레임에서 그려야 할 화면 수다.

**답은 내가 먼저 쓴다. 담당자에게 다섯 개를 물어보는 절차가 아니다.**
...
"이렇게 가정했다"를 붙여 ⑤ 구조 컨펌에서 함께 확인받는다.

아무것도 안 하고 다섯 개를 되묻지 않는다. 물어야 할 게 있으면 **먼저 할 수 있는 것을 다 해놓고**
그 지점에서 묻는다.
...
문구를 새로 짓기 전에, **그 자리에 이미 쓰이는 말**부터 실제 제품에서 찾는다. 이 단계가 없으면 톤 규칙을 아무리 잘 지켜도 출고된 앱과 다른 말이 나오고, 개발자는 "이거 의도한 변경이냐"고 되묻게 된다. 그 확인은 결국 담당자와 개발자의 시간으로 치른다.

작업은 보통 화면 몇 개만 건드리므로 대조도 두 겹이면 충분하다.

| 겹 | 확인할 것 | 방법 |
|---|---|---|
| 1겹 — 같은 자리 | 지금 고치는 화면에 이미 쓰이는 문구 | ⓪에서 최신화한 구현 소스와 baseline 캡처에서 그 화면을 찾아 문구를 그대로 읽는다 |
| 2겹 — 같은 개념 | 이 화면에 **새로 등장하는 개념**을 다른 화면은 뭐라 부르는지 | 구현 소스 전체를 개념어로 검색한다 |

- 소스·캡처 위치는 `docs/design-harness.md`가 가리킨다. ⓪에서 이미 최신화해 뒀으므로 추가 준비는 없다.
- 1겹에서 문구를 찾았으면 **그것이 기본값**이다. 바꿀 이유가 있을 때만 바꾼다.
- 대조 대상이 없는 새 화면의 새 문구는 그냥 ux-writing 스킬 규칙대로 쓴다.
- 미리 만든 문구 사전은 두지 않는다 — 앱이 문구를 바꾸는 순간 사전이 거짓말을 시작한다.

**달라진 문구는 목록으로 모아 둔다.** ⑤와 ⑩ 두 번 담당자 확인에 쓴다.

| 자리 | 기존 앱 | 변경안 | 이유 |
|---|---|---|---|
| [화면·요소] | [출고 중인 문구] | [새 문구] | [왜 바꾸는지] |

달라진 게 없으면 표를 만들지 않는다. 아무 말도 없으면 "기존 그대로"라는 뜻이어야 읽는 사람이 안심한다.
...
| "더 나은 문구가 떠올랐으니 그냥 쓰면 되지" | 더 나은지는 담당자가 정한다. 기존 문구를 바꾸는 건 제품을 바꾸는 것이다. 대조하고 목록에 올려라. |
...
**B. UI·레이아웃:** 값은 여기 적지 않는다 — **디자인 시스템이 주인**이고, 그 문서를 열어 대조한다
(Dou는 `docs/design-system/DESIGN.md`).

- **터치·안전영역** — DESIGN.md §12 기준으로 확인
- **접근성** — 대비·포커스·색만으로 전달 금지. DESIGN.md §11 기준
- **모션** — 지속시간·이징이 DESIGN.md §10의 단계 안에 있는가, 모션 줄이기를 존중하는가
...
| 자리 | 기존 앱 | 변경안 | 이유 |
|---|---|---|---|
[② 대조에서 모은 목록. 달라진 게 없으면 이 섹션 자체를 뺀다]

→ 이 구조로 구현을 진행할까요?
```

문구 변경을 여기서 한 번 걸러야 하는 이유는 비용이다. 구현·핸드오프까지 간 뒤에 "이건 왜 바꿨냐"가 나오면 화면과 문서를 되돌려야 한다. 구조 컨펌 자리에서는 표 한 줄 지우면 끝난다.
...
1. **주 흐름을 끝까지 눌러 본다.** 시작부터 완료까지. 중간에서 뒤로 가 보고, 취소해 보고,
   다시 들어와 본다.
2. **상태를 하나씩 열어 본다.** ②-0에서 적은 상태 목록 그대로. 빈·로딩·실패·권한 없음이
...
연결하면 목록에 그대로 남아 있다" ○). 고칠 수 있는 건 고치고 넘기고, 판단이 필요한 건 목록으로
올린다.
...
- ② 대조에서 모은 **문구 변경 목록이 있으면 이 표 아래에 함께 붙인다.** 구현하면서 문구가
  더 바뀌었으면 그것도 목록에 더한다 — ⑤에서 한 번 봤다고 넘어가지 않는다.
...
② 대조에서 모은 목록에 구현 중 바뀐 것까지 더해 최종본을 담당자에게 보이고 확인받는다.
**확인받지 않은 문구 변경은 커밋하지 않는다.** 여기가 마지막 관문이라, 여기를 통과한 문구는
개발 핸드오프 이슈의 `문구 변경` 섹션에 그대로 들어간다(`saylog-uxui-issues` 참조).

달라진 문구가 없으면 아무것도 묻지 않는다 — 없는 걸 확인시키는 건 담당자 시간만 쓴다.
...
| 기존 앱 문구 대조 없이 새 문구 작성 | 이미 있는 화면인데 라벨·상태 문구를 새로 지음 | ②에서 구현 소스·baseline 캡처로 기존 문구를 먼저 찾는다. 바꿀 거면 변경 목록에 올린다 |
