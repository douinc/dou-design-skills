...
구현(또는 피드백 반영)을 마치고 담당자 확인 차례가 되면, URL을 말로만 전하지 말고
**확인할 화면의 URL을 담당자 기본 브라우저로 직접 연다** (macOS는 `open "<url>"`).
담당자는 디자이너다 — 주소를 복사해 붙여넣게 하는 것 자체가 확인 진입 장벽이다.
dev 서버가 꺼져 있으면 먼저 띄운 뒤 연다. 같은 세션에서 재확인을 요청받으면 다시 연다.
